    ...
    for line in lines:
        ...
        if line:
            ...
            if nid == 1:
                ...
                if '\t' in line:
                    # Process turn containing user utterance and bot response
                    u, r = line.split('\t')
                    a = candid_dic[r]
                    u = tokenize(u)
                    r = tokenize(r)
                    data.append((profile[:], user_utterances[:], bot_responses[:], kb_facts[:], u[:], a))
                    u.append('$u')
                    u.append('#' + str(nid))
                    r.append('$r')
                    r.append('#' + str(nid))
                    user_utterances.append(u)
                    bot_responses.append(r)
                else:
                    # Process turn containing KB fact
                    f = tokenize(line)
                    f.append('$f')
                    f.append('#' + str(nid))
                    kb_facts.append(f)
...
def vectorize_data(data, word_idx, sentence_size,
    ...
    for i, (profile, user_utterances, bot_responses, kb_facts, query, answer) in enumerate(data):
        if i % batch_size == 0:
            memory_size = max(1, min(max_memory_size, len(bot_responses)))
        ...
        uu = []
        for i, sentence in enumerate(user_utterances, 1):
            lu = max(0, sentence_size - len(sentence))
            uu.append([word_idx[w] if w in word_idx else 0 for w in sentence] + [0] * lu)

        bb = []
        for i, sentence in enumerate(bot_responses, 1):
            lb = max(0, sentence_size - len(sentence))
            bb.append([word_idx[w] if w in word_idx else 0 for w in sentence] + [0] * lb)

        ff = []
        for i, sentence in enumerate(kb_facts, 1):
            lf = max(0, sentence_size - len(sentence))
            ff.append([word_idx[w] if w in word_idx else 0 for w in sentence] + [0] * lf)

        # Take only the most recent sentences that fit in memory
        uu = uu[::-1][:memory_size][::-1]
        bb = bb[::-1][:memory_size][::-1]
        ff = ff[::-1][:memory_size][::-1]

        # Pad to memory_size
        to_pad = max(0, memory_size - len(uu))
        for _ in range(to_pad):
            uu.append([0] * sentence_size)

        to_pad = max(0, memory_size - len(bb))
        for _ in range(to_pad):
            bb.append([0] * sentence_size)

        to_pad = max(0, memory_size - len(ff))
        for _ in range(to_pad):
            ff.append([0] * sentence_size)

        lq = max(0, sentence_size - len(query))
        q = [word_idx[w] if w in word_idx else 0 for w in query] + [0] * lq

        P.append(np.array(pp))
        U.append(np.array(uu))
        B.append(np.array(bb))
        F.append(np.array(ff))
        Q.append(np.array(q))
